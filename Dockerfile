...
RUN apt-get -y install gcc make autoconf libc-dev pkg-config libzip-dev git patch
...
#RUN /bin/bash -c 'curl -sSL https://phar.phpunit.de/phpunit.phar -o phpunit.phar' && \
#chmod +x phpunit.phar && \
#mv phpunit.phar /usr/local/bin/phpunit && \
# Install drush
RUN curl -Lo /usr/local/bin/drush https://github.com/drush-ops/drush/releases/download/8.3.0/drush.phar && \
...
COPY "memory_limit_php.ini" "/usr/local/etc/php/conf.d/memory-limit-php.ini"
